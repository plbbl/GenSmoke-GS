...
class DepthGuidedFusionModule(nn.Module):
    def __init__(self, in_channels=4, out_channels=64):
        ...
        # 卷积块：处理 4 通道数据（RGB + 深度图）
        self.conv_block = nn.Sequential(
            nn.Conv2d(in_channels, out_channels, kernel_size=3, padding=1),  # 输入4通道，输出out_channels通道
            nn.InstanceNorm2d(out_channels),  # 实例归一化
            nn.GELU()  # 激活函数
        )
    ...
    def forward(self, x):
        ...
        # 使用语义选择注意力机制生成通道重要性权重
        attention_weights = self.semantic_attention(conv_out)
        attention_weights = attention_weights.unsqueeze(-1).unsqueeze(-1)  # 扩展维度以匹配空间
        attention_out = x * attention_weights  # 使用注意力权重调整特征
